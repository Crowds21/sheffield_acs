...
- ((64073fe6-0f5a-4952-8a4a-4b70f50c1fea)) 条件随机场 CRF
  ref:: [从隐马尔科夫到条件随机场 | MaxMa Blog (anxiang1836.github.io)](https://anxiang1836.github.io/2019/11/05/NLP_From_HMM_to_CRF/)
	...
	- Introduction
		...
		- $\mathbf{w}$ 是参数向量，它用于对特征函数 $\phi(y_n, y_{n-1}, \mathbf{x}, n)$ 进行加权求和。特征函数 $\phi(y_n, y_{n-1}, \mathbf{x}, n)$ 是一组根据当前标签 $y_n$、前一个标签 $y_{n-1}$、输入序列 $\mathbf{x}$ 和位置 $n$ 计算得到的特征。这些特征可以根据具体的问题和任务进行设计，用于捕捉输入序列和输出标签之间的关联关系。
		-
